...
export const useSignUpForm = () => {
  ...
  return useMutation<SignUpResponse, BadLoginResponse, SignUpParams>({
    ...
    onSuccess: (_, variables) => {
      ...
      // Disparamos el pixel enviando el email que usó para registrarse
      trackPixelEvent('both', 'CompleteRegistration', {
        content_name: 'App Registration',
        status: 'success',
        email: variables.email, // Importante para que el pixel haga match del usuario
      });
    },
    onError: (error) => {
